...
@Component({
  selector: 'app-modalaffectationpanier',
  templateUrl: './modalaffectationpanier.page.html',
  styleUrls: ['./modalaffectationpanier.page.scss'],
})
export class ModalaffectationpanierPage implements OnInit {


  listchauffeurtaxi = [];
  listchauffeurmedical = [];
  public filterValue;
  ...
  getchauffeurappartienta(idcourse) {
    this.listchauffeurtaxi = [];
    this.listchauffeurmedical = [];
    this.courseService.getCoursebyId(idcourse).subscribe((res2: any) => {
      ...
      this.userservices.getallchauffeurpartype({
        ...
        this.userservices.getallchauffeurpartype({
          ...
          this.listchauffeurtaxi = res.concat(res1);
