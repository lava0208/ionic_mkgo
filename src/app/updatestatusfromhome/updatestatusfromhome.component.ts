...
@Component({
    selector: 'app-updatestatusfromhome',
    templateUrl: './updatestatusfromhome.component.html',
    styleUrls: ['./updatestatusfromhome.component.scss'],
})
export class UpdatestatusfromhomeComponent implements OnInit {
    ...
    constructor(private navParams: NavParams, private toastController: ToastController, public navCtrl: NavController,
                private geolocation: Geolocation, private userservices: UserService, private clientService: ClientService,
                private popoverController: PopoverController, private router: Router, public actionSheetCtrl: ActionSheetController,
                private formBuilder: FormBuilder, private cd: ChangeDetectorRef, public camera: Camera,
        ...
        await this.courseService.getaffectbyId(id).then(async res => {
            ...
            await this.courseService.getcoursebyId(res['course'].replace('/api/courses/', '')).then((res1: any) => {
                ...
                if (res1.entreprise) {
                    ...
                    this.course.userid =    this.course.userid = localStorage.getItem('userid');
    ...
    addCourseClientEdit(index, id, client?, destination?, depart?, tel?, securitesocial?, typemutuelle?, distance?, duree?, value?) {
        ...
        this.courseClients1.tel = tel;
        this.courseClients1.distance = distance;
        this.courseClients1.duree = duree;
        this.courseClients1.value = value;
        this.courseClients1.securitesocial = securitesocial;
        this.courseClients1.typemutuelle = typemutuelle;
